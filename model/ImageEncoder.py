...
class ImageEncoder(nn.Module):
    def __init__(self, pretrained_dir, image_encoder='base'):
        ...
        # directory is fine
        if image_encoder in ['vit-base']:

            self.tokenizer = tokenizer.from_pretrained(pretrained_dir+'/vit-base-224/')
            self.model = model.from_pretrained(pretrained_dir+'/vit-base-224/')
        else:

            self.tokenizer = tokenizer.from_pretrained(pretrained_dir+'/vit-large-224/')
            self.model = model.from_pretrained(pretrained_dir+'/vit-large-224/')
